// All rights reserved.
// Use of this source code is governed by a GNU General Public License v3.0 that can be
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Widget build(BuildContext context) {
    return new SplashScreen(
      ...
      title: new Text(
        '\nSudoku',
        style: new TextStyle(fontWeight: FontWeight.bold, fontSize: 25.0, color: Styles.fg),
      ),
...
class AfterSplashState extends State<AfterSplash> {
  ...
  void checkResult() {
    if (game.toString() == gameSolved.toString()) {
      ...
      Timer(Duration(milliseconds: 500), () {
        ...
        showAnimatedDialog<void>(animationType: DialogTransitionType.fadeScale, barrierDismissible: true, duration: Duration(milliseconds: 350), context: context, builder: (_) => AlertGameOver()).whenComplete(() {
  ...
  static List<List<List<int>>> getNewGame([String difficulty = 'easy']) {
    ...
    return [
      object.newSudoku,
      object.newSudokuSolved
    ];
  }

  void setGame(int mode, [String difficulty = 'easy']) {
    if (mode == 1) {
      game = new List.generate(
          9,
          (i) => [
                0,
                0,
        ...
        child: FlatButton(
          onPressed: isButtonDisabled || gameCopy[k][i] != 0
              ...
              : () {
                  ...
                  showAnimatedDialog<void>(animationType: DialogTransitionType.fade, barrierDismissible: true, duration: Duration(milliseconds: 300), context: context, builder: (_) => AlertNumbersState()).whenComplete(() {
                    callback([
                      k,
                      i
                    ], AlertNumbersState.number);
          ...
          textColor: game[k][i] == 0 ? buttonColor(k, i) : Styles.secondaryColor,
  ...
  Widget buildFloatingActionButton() {
    final TextStyle customStyle = TextStyle(
        ...
        color: Colors.black);
    final icons = [
      SpeedDialAction(backgroundColor: Styles.bg_2, child: Icon(Icons.refresh), label: Text('Restart Game', style: customStyle)),
      SpeedDialAction(backgroundColor: Styles.bg_2, child: Icon(Icons.add_rounded), label: Text('New Game', style: customStyle)),
      SpeedDialAction(backgroundColor: Styles.bg_2, child: Icon(Icons.lightbulb_outline_rounded), label: Text('Show Solution', style: customStyle)),
      ...
      SpeedDialAction(backgroundColor: Styles.bg_2, child: Icon(Icons.info_outline_rounded), label: Text('About', style: customStyle)),
    ];
      ...
      case 3:
        {
          ...
          Timer(Duration(milliseconds: 300), () {
            ...
            showAnimatedDialog<void>(animationType: DialogTransitionType.fadeScale, barrierDismissible: true, duration: Duration(milliseconds: 350), context: context, builder: (_) => AlertDifficultyState(currentDifficultyLevel)).whenComplete(() {
      ...
      case 5:
        {
          ...
          Timer(Duration(milliseconds: 300), () {
            ...
            showAnimatedDialog<void>(animationType: DialogTransitionType.fadeScale, barrierDismissible: true, duration: Duration(milliseconds: 350), context: context, builder: (_) => AlertAbout()).whenComplete(() {
              setState(() {
                isFABDisabled = false;
              });
            });
    ...
    return new WillPopScope(
        onWillPop: () async {
          if (kIsWeb) {
            ...
            showAnimatedDialog<void>(animationType: DialogTransitionType.fadeScale, barrierDismissible: true, duration: Duration(milliseconds: 350), context: context, builder: (_) => AlertExit()).whenComplete(() {
              setState(() {
                isFABDisabled = false;
              });
            });
